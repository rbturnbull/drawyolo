...
def drawyolo(
    ...
    width:int=typer.Option(None, help="Width of the output image if you wish to resize it"),
    height:int=typer.Option(None, help="Height of the output image if you wish to resize it"),
    line_thickness:int=typer.Option(None, help="Thickness of the box lines. If not provided, it will be calculated based on the final image size."),
):
    """
    ...
    if labels is not None:
        draw_box_on_image_with_labels(image, labels, output, classes, width=width, height=height, line_thickness=line_thickness)
    elif weights is not None:
        draw_box_on_image_with_model(image, weights, output, classes=classes, highest=highest, width=width, height=height, line_thickness=line_thickness)
    else:
